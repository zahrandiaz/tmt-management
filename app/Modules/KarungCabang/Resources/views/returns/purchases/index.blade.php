                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('karungcabang.dashboard') }}" class="btn btn-secondary btn-sm"><i class="bi bi-arrow-left-circle"></i> Kembali ke Dashboard</a>
                        </div>
                        <div class="card-body">
                                    ...
                                    <tbody>
                                        @forelse ($returns as $return)
                                            <tr>
                                                ...
                                                <td><a href="{{ route('karungcabang.purchases.show', $return->originalTransaction->id) }}">{{ $return->originalTransaction->purchase_code }}</a></td>
                                                <td>{{ $return->supplier->name ?? 'N/A' }}</td>
                                                <td>{{ $return->return_date->format('d M Y') }}</td>
                                                <td class="text-end">Rp {{ number_format($return->total_amount, 0, ',', '.') }}</td>
                                                <td class="text-center">
                                                    <a href="{{ route('karungcabang.returns.purchases.show', $return->id) }}" class="btn btn-info btn-sm text-white" title="Lihat Detail">
