...
<x-app-layout>
    ...
    <x-module-layout>
        ...
        <div class="container-fluid" x-data="returnFormHandler()">
            <form action="{{ route('karungcabang.purchases.returns.store', $purchaseTransaction->id) }}" method="POST">
                @csrf
                <div class="card">
                    <div class="card-header bg-warning text-dark d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Buat Retur untuk Pembelian #{{ $purchaseTransaction->purchase_code }}</h5>
                        <a href="{{ route('karungcabang.purchases.show', $purchaseTransaction->id) }}" class="btn btn-secondary btn-sm"><i class="bi bi-x-circle"></i> Batal</a>
                    </div>
                    <div class="card-body">
                        @if ($errors->any())
