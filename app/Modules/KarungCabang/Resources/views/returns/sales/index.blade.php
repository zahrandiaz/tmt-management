                                    ...
                                    <tbody>
                                        @forelse ($returns as $return)
                                            <tr>
                                                ...
                                                <td><a href="{{ route('karungcabang.sales.show', $return->originalTransaction->id) }}">{{ $return->originalTransaction->invoice_number }}</a></td>
                                                <td>{{ $return->customer->name ?? 'N/A' }}</td>
                                                <td>{{ $return->return_date->format('d M Y') }}</td>
                                                <td class="text-end">Rp {{ number_format($return->total_amount, 0, ',', '.') }}</td>
                                                <td class="text-center">
                                                    <a href="{{ route('karungcabang.returns.sales.show', $return->id) }}" class="btn btn-info btn-sm text-white" title="Lihat Detail">
