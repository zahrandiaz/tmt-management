            ...
            <div class="card">
                <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
                    ...
                    <div class="no-print">
                        @if($purchase->status == 'Completed')
                            ...
                            <form action="{{ route('karung.purchases.cancel', $purchase->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan transaksi ini?');">
                                @csrf
                            ...
                            <form action="{{ route('karung.purchases.destroy', $purchase->id) }}" method="POST" class="d-inline" onsubmit="return confirm('PERINGATAN! Menghapus transaksi akan menyembunyikannya dari daftar dan bersifat permanen. Lanjutkan?');">
                                @csrf
                                @method('DELETE')
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-6"><p class="mb-1"><strong>No. Referensi:</strong> {{ $purchase->purchase_reference_no ?: '-' }}</p><p class="mb-1"><strong>Dicatat Oleh:</strong> {{ $purchase->user?->name ?: 'N/A' }}</p><p class="mb-1"><strong>Status:</strong> @if($purchase->status == 'Cancelled')<span class="badge bg-danger">{{ $purchase->status }}</span>@else<span class="badge bg-success">{{ $purchase->status }}</span>@endif</p></div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>@foreach ($purchase->details as $index => $detail)<tr class="{{ $purchase->status == 'Cancelled' ? 'text-muted' : '' }}"><th scope="row">{{ $index + 1 }}</th><td class="{{ $purchase->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">{{ $detail->product?->name ?: 'Produk Telah Dihapus' }}</td><td class="text-center {{ $purchase->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">{{ $detail->quantity }}</td><td class="text-end {{ $purchase->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">Rp {{ number_format($detail->purchase_price_at_transaction, 0, ',', '.') }}</td><td class="text-end {{ $purchase->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">Rp {{ number_format($detail->sub_total, 0, ',', '.') }}</td></tr>@endforeach</tbody>
                            <tfoot><tr class="table-dark"><th colspan="4" class="text-end">TOTAL PEMBELIAN</th><th class="text-end {{ $purchase->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">Rp {{ number_format($purchase->total_amount, 0, ',', '.') }}</th></tr></tfoot>
                        </table>
                    </div>
