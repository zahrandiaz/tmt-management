...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="h4 fw-bold mb-0">
            {{-- Judul ini diambil dari @section('title') di kode lama Anda --}}
            Dashboard Modul Toko Karung
        </h2>
    </x-slot>

    {{-- Memanggil komponen layout 2 kolom (sidebar + content) yang sudah kita buat --}}
    <x-module-layout>
        
        {{-- Mengisi slot 'sidebar' dengan file partial yang sudah ada --}}
        <x-slot name="sidebar">
            @include('karung::layouts.partials.sidebar')
        </x-slot>

        {{-- Ini adalah KONTEN ASLI dari file Anda, yang sebelumnya ada di dalam @section('module-content') --}}
        {{-- Kita pindahkan seluruh isinya ke sini --}}
    ...
    {{-- [PENTING] Menempatkan script khusus halaman di slot terpisah --}}
    {{-- Ini adalah pengganti dari @push('footer-scripts') --}}
    @if (!isset($scripts))
    <x-slot name="scripts">
    @endif
        <script>
