                        ...
                        <table class="table table-striped table-hover table-bordered">
                            ...
                            <tbody>
                                @forelse ($sales as $sale)
                                    <tr class="{{ $sale->status == 'Cancelled' ? 'table-secondary text-muted' : '' }}">
                                        <td>{{ $sale->transaction_date->format('d-m-Y H:i') }}</td>
                                        <td>{{ $sale->invoice_number }}</td>
                                        <td>{{ $sale->customer?->name ?: 'Penjualan Umum' }}</td>
                                        <td>
                                            <span class="{{ $sale->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">
                                        ...
                                        <td class="text-center">
                                            ...
                                            {{-- [MODIFIKASI] Tombol Edit hanya muncul untuk yang berhak --}}
                                            ...
                                            @if($sale->status == 'Completed')
                                            <form action="{{ route('karung.sales.cancel', $sale->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan transaksi ini? Aksi ini tidak dapat diurungkan.');">
                                                @csrf
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center">Tidak ada data transaksi penjualan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
