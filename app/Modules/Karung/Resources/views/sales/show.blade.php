            ...
            <div class="card">
                    ...
                    <div class="no-print">
                        ...
                        @if($sale->status == 'Completed')
                            @can('karung.edit_sales')
                                <a href="{{ route('karung.sales.edit', $sale->id) }}" class="btn btn-warning btn-sm" title="Edit Transaksi">
                            ...
                            <form action="{{ route('karung.sales.cancel', $sale->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan transaksi ini?');">
                                @csrf
                            ...
                            <form action="{{ route('karung.sales.destroy', $sale->id) }}" method="POST" class="d-inline" onsubmit="return confirm('PERINGATAN! Menghapus transaksi akan menyembunyikannya dari daftar. Lanjutkan?');">
                                @csrf
                                @method('DELETE')
                ...
                <div class="card-body">
                    @if($sale->status == 'Cancelled')
                        <div class="alert alert-danger"><strong>Transaksi Dibatalkan!</strong> Transaksi ini telah dibatalkan dan tidak lagi dihitung dalam laporan.</div>
                    @endif
                    <div class="row mb-4">
                        <div class="col-md-6"><p><strong>No. Invoice:</strong> {{ $sale->invoice_number }}</p><p><strong>Tanggal Transaksi:</strong> {{ $sale->transaction_date->format('d F Y, H:i') }}</p><p><strong>Status:</strong> @if($sale->status == 'Cancelled')<span class="badge bg-danger">{{ $sale->status }}</span>@else<span class="badge bg-success">{{ $sale->status }}</span>@endif</p></div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>@foreach ($sale->details as $index => $detail)<tr class="{{ $sale->status == 'Cancelled' ? 'text-muted' : '' }}"><th scope="row">{{ $index + 1 }}</th><td class="{{ $sale->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">{{ $detail->product?->name ?: 'Produk Telah Dihapus' }}</td><td class="text-center {{ $sale->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">{{ $detail->quantity }}</td><td class="text-end {{ $sale->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">Rp {{ number_format($detail->selling_price_at_transaction, 0, ',', '.') }}</td><td class="text-end {{ $sale->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">Rp {{ number_format($detail->sub_total, 0, ',', '.') }}</td></tr>@endforeach</tbody>
                            <tfoot><tr class="table-dark"><th colspan="4" class="text-end">TOTAL PENJUALAN</th><th class="text-end {{ $sale->status == 'Cancelled' ? 'text-decoration-line-through' : '' }}">Rp {{ number_format($sale->total_amount, 0, ',', '.') }}</th></tr></tfoot>
                        </table>
                    </div>
                </div>
            </div>
