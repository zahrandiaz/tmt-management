...
@section('title', 'Edit Jenis Produk - Modul Toko Karung')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-warning text-dark">
                    <h5 class="mb-0">Edit Jenis Produk: {{ $productType->name }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('karung.product-types.update', $productType->id) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="name" class="form-label">Nama Jenis <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $productType->name) }}" required>
                            @error('name')
                        ...
                        {{-- business_unit_id tidak perlu diubah di sini --}}
